...
[this needs to come before RVIZ. Maybe introduction, maybe put the whole debugging in ros before the graphical tools. explain the computation model because it is important for the tools: pub/sub]
...
rxconsole is a graphical user interface to display messages that have been published on the special purpose topic \emph{/rosout}. The special purpose topic \emph{/rosout} is used by the ROS logging framework (see Section~\ref{ros_logging}). rxconsole keeps a list of messages with additional information like the severity of the message, the node which published the message and the timestamp when the message was published. The tool also allows filtering by severity and text.
...
This section presents the logging facilities in ROS, which is similar to many other logging frameworks not only in robotics. Other popular logging frameworks are Apache's Log4j\footnote{\url{http://logging.apache.org/log4j}} for Java and log4cxx\footnote{\url{http://logging.apache.org/log4cxx/}} for C++. This last part of this section presents the most basic form of logging: print statements.

[Should I go into more detail with log4j and log4cxx?]
...
Since all log messages are published on the \emph{/rosout} topic if the log level is set to debugging, it is easy to access them even if a node is running on a different machine. The graphical user interface rxconsole can be used to display and filter log messages by severity and full-text search.

\subsection{Logging in Player/Stage}
[should I really write about player/stage and orca? I don't know much about their logging facilities but it looks more like a "log communication to file" thing...]
