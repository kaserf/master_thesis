...
\q{where should I introduce the word dashboard and widget? goals? user interface? ---> user interface}
...
\todo{Discuss architecture and how it interacts with an eventual robot framework. All of them probably have some sort of inter process communication or a communication middleware in general, which can be accessed by the visualization tool.}
...
\q{Should I copy the screenshot from \cite{Gumbley2009} or only cite it?}

\todo{Write about the communication setup here, since it is referenced in the architecture.}
...
\todo{write about modular visualizations, wrapped in widgets; the central dashboard component knows about the widgets but all the preferences and subscription details are stored in the widget to reduce the coupling and make the widgets independent; how data is accessed (maybe abstract from pub sub and show a component diagram of the dashboard system with the robot framework, connected on a middleware}
