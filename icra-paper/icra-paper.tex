...
This section presents important implementation details. First the object model is presented: it has been designed to allow easy integration of a plugin framework in the future (see section \ref{future}). Topic introspection is used to keep the configuration overhead low, which has been identified as a core requirement.
...
Although the current implementation should still be considered as a prototype, it has all the features that were initially planned. The implementation is running stable and first attempts to use it as a debugging tool have been made. ROSDashboard is ready to be used for the development of robot applications and can be used for further evaluations (see \ref{future}).

Figure \ref{showcase} shows a simple showcase scenario: ROSDashboard is running alongside the turtlesim\_node which is used in many examples in the ROS tutorials\footnote{http://www.ros.org/wiki/ROS/Tutorials}. It monitors the values for linear and angular speed which are published by the turtle\_teleop\_key node to control the turtle simulation. The String widget is configured to display messages from \emph{/rosout}, which in this example shows a warning when the turtle hits a wall.
...
\begin{figure}[thpb]
  \centering
  \framebox{
    \includegraphics[scale=0.26]{images/showcase2.png}
  }  
  \caption{ROSDashboard running alongside turtlesim\_node.}
  \label{showcase}
\end{figure}

Figure \ref{rosgraph} shows the ROS computation graph during the execution of the showcase scenario. It shows how ROSDashboard is connected to the nodes which are debugged. The topics needed for the showcase are \emph{/turtle1/command\_velocity} for the linear and angular velocity and \emph{/rosout} for warnings when the turtle hit the wall.
...
\begin{figure}[thpb]
  \centering
  \framebox{
    \includegraphics[scale=0.3]{diagrams/rosgraph}
  }  
...
% future work
The presented tool is a first prototype to evaluate the use of a simple visualization tool during debugging. Further work can be done to increase the number of available widgets. Ultimately there should be a plugin engine where third party developers can easily add widgets for new and more complex kinds of data. The extendibility of the tool was one of the initial requirements and the structure of the program allows easy integration of such a plugin engine.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
																																																																																																																															...
																																																																																																																															This work is part of the final project in the Software Engineering Elite Graduate Program at Universität Augsburg, TU München and LMU München\footnote{http://www.studieren.se}. It was conducted at the Department of Electrical and Computer Engineering, University of Auckland, New Zealand.
