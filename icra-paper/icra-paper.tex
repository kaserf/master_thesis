...
ARDev is an augmented reality (AR) debugging system based on the Player/Stage system \cite{Collett2010, Collett2007, Collett2006}. The AR approach to debugging helps the developer to understand the global state of the robot by augmenting a video feed of the robot with information of the robots' perception. It also focuses on pre-defined data such as laser sensors and the like, visualizing abstract data has been identified as possible future work \cite{Collett2010}. Initial evaluation studies were promising that the visualization can help developers significantly during debugging \cite{Collett2010}.

3D visualizations like rviz and ARDev are really useful to understand the robot-view of the world. Both of them need a substantial amount of work to set up. For rviz a 3D model of the robot needs to be created, which has the exact proportions of the real robot. ARDev requires an even more complex setup for the augmented reality part. During the development of the tool an intelligent debugging space (IDS) was permanently installed in a laboratory \cite{Collett2010}, where the robot under development was constantly tracked with markers and cameras. This setup is often unavailable, especially in outdoor situations, and is also often not needed for debugging tasks where only abstract data is available. A general problem with 3D visualization tools is that abstract data often does not fit into the 3-dimensional rendering and in case of augmented reality can hide important information from the real world \cite{Collett2010}.

\subsection{LabVIEW}

LabVIEW (Laboratory Virtual Instrumentation Engineering Workbench) is a graphical programming environment developed by National Instruments\footnote{www.ni.com}. It features \emph{Front Panels} which allows a developer to add widgets to a panel. These widgets can display the status of the graphical program or act as control interface (button, knobs, etc.) for the program. For each widget on the front panel an element is created in the block diagram of the program which can be connected to other parts in the block diagram. LabVIEW delivers a high number of pre-defined widgets like string displays, sliders and progress bars, which can be used to create a dedicated user interface for a application. This user interface can be used to control the application as well as debug the application.
...
[what are the ways of getting data? roslogging and tracepoints?]

[what is ros and how can we make use of logging and topics]
