...
To address the special kind of data robots usually handle, the current debugging tools need to be adapted. Existing debugging tools for traditional software systems do not visualize data in their natural representation, they render everything as text. While this is a good approach for traditional systems, it fails with robotic applications. When debugging traditional systems the developer usually has enough time to interpret the data, since they can interrupt the execution and step through the program code. This is not possible for robot applications because robots generally don't run in a deterministic and suspendable environment\cite{Gumbley2009}. Smart visualization can help the developer to understand the data with less cognitive overhead and thus faster.
